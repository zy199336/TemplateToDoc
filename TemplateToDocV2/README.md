...
```powershell
cd C:\Users\yzhen\Downloads\shigongfangan
python -m pip install -e .\TemplateToDocV2
```

运行测试：

```powershell
python -m pytest TemplateToDocV2\tests
```
...
```text
TemplateToDocV2/profiles/atc_sdd/reference.docx
TemplateToDocV2/profiles/atc_sdd/profile.yaml
TemplateToDocV2/projects/atc_sdd_demo/project.yaml
```

如果没有安装为命令行脚本，也可以直接运行源码入口：

```powershell
python TemplateToDocV2\src\template_to_doc_v2\cli.py import-template .\空管自动化软件设计说明_扩写版.docx --profile atc_sdd --project-id atc_sdd_demo
```
...
```text
TemplateToDocV2/projects/atc_sdd_demo/final.docx
TemplateToDocV2/projects/atc_sdd_demo/replacement_report.yaml
```
...
```text
TemplateToDocV2/projects/atc_sdd_demo/qa_compare.yaml
TemplateToDocV2/projects/atc_sdd_demo/render/candidate/
TemplateToDocV2/projects/atc_sdd_demo/render/reference/
TemplateToDocV2/projects/atc_sdd_demo/render/diff/
```
...
下面的示例以 [空管自动化软件设计说明_扩写版.docx](../空管自动化软件设计说明_扩写版.docx) 作为模板。运行命令后会在本地创建 `TemplateToDocV2/projects/atc_sdd_demo`，生成 `final.docx`，并将部分页面渲染成 PNG，便于快速查看效果。
...
TemplateToDocV2/projects/atc_sdd_demo/replacement_report.yaml
TemplateToDocV2/projects/atc_sdd_demo/qa_compare.yaml
TemplateToDocV2/projects/atc_sdd_demo/render/candidate/final.pdf
TemplateToDocV2/projects/atc_sdd_demo/render/candidate/final-01.png
TemplateToDocV2/projects/atc_sdd_demo/render/candidate/final-03.png
TemplateToDocV2/projects/atc_sdd_demo/render/candidate/final-05.png
```
...
- V2 会复用 `../TemplateToDoc/src/template_to_doc` 中稳定的 QA 渲染比较能力，但不会修改原 `TemplateToDoc/` agent。
...
The following example uses [空管自动化软件设计说明_扩写版.docx](../空管自动化软件设计说明_扩写版.docx) as the template. Running the commands creates `TemplateToDocV2/projects/atc_sdd_demo` locally, builds `final.docx`, and renders selected pages to PNG for quick inspection.
...
- V2 reuses stable QA rendering utilities from `../TemplateToDoc/src/template_to_doc` when available, but it does not modify the existing `TemplateToDoc/` agent.
